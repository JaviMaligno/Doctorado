...
We assume that the reader is familiar with the basic terminology of category theory. For an introduction to this topic we refer the reader to \cite{maclane}. Here we briefly recall the notion of symmetric monoidal categories and several versions of monoidal functors. The detailed definitions with all the precise diagrams can also be found in \cite{maclane}.
...
for all objects $x,y,z\in\CC$ called \emph{associators}, a natural isomorphism
\[\lambda_x:1\otimes x\to x\]
for every $x\in\CC$ called \emph{left unitor}, a natural isomorphism
\[\rho_x:x\otimes 1\to x\]
for every $x\in\CC$ called \emph{right unitor}, and a natural isomorphism
\[\gamma_{x,y}:x\otimes y \to y\otimes x\]
for all $x,y\in\CC$ called \emph{braiding} or \emph{symmetry isomorphism}. These morphisms satisfy natural unitality and associativity axioms.
\end{defin}

\begin{defin}
Let $(\CC,\otimes_\CC, 1_\CC)$ and $(\DD, \otimes_\DD, 1_\DD)$ be symmetric monoidal categories. A \emph{(lax) monoidal functor} between tem is a functor
\[F:\CC\to\DD\]
with a morphism
\[\varepsilon:1_\DD\to F(1_\CC)\]
and a natural transformation
\[\mu_{x,y}:F(x)\otimes_\DD  F(y)\to F(x\otimes_\CC y)\]
for all $x,y\in\CC$ satisfying natural unitality associativity and symmetry axioms. A lax monoidal functor is called \emph{strong monoidal} if $\varepsilon$ and $\mu_{x,y}$ are isomorphisms for all $x,y\in\CC$.
\end{defin}
...
\[
\begin{tikzcd}
F(x)\otimes_\DD F(y)\arrow[r, "\alpha_x\otimes_\DD\alpha_y"]\arrow[d,"\mu_{x,u}"] & G(x)\otimes_\DD G(y)\arrow[d, "\nu_{x,y}"]\\
F(x\otimes_\CC y)\arrow[r, "\alpha_{x\otimes_\CC y}"] & G(x\otimes_\DD y)
\end{tikzcd}
\]
