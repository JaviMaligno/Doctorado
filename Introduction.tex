...
\end{tikzcd}   
   ...
   By naturality of $\mu$ we  have that \[\mu\circ (F(\circ_i)\otimes id)= F( \circ_i \otimes iid)\circ \mu\] and \[\mu\circ (id \otimes F(\circ_{j-i+1}))= F(id\otimes \circ_{j-i+1})\circ \mu.\]
...
\end{tikzcd}   
   \]
   Now, the top diagram commutes because it is the hexagon axiom of lax monoidal functors. The bottom diagram is precisely diagram (\ref{assaux}), so it commutes and we get the desired associativity axiom.
