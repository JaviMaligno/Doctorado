...
Let $R$ be a ring of characteristic distict of 2. 
...
\item[$\ast$]<4-> A chain map is a degree 0 map $f:V\to W$ such that $fd_V=d_Wf$. %usually they must be of degree 0, that's how the appear in topology, but one could let more degrees and would obtain different things, even getting a chain complex on the morphisms
\item[]<5-> Example:
\[
\begin{tikzcd}
...
\begin{itemize}
\item<1-> \textbf{Symmetric monoidal category of chain complexes}
\item[$\ast$]<2-> Same tensor product $(V\otimes W)^n=\bigoplus_{p+q=n} V^p\otimes W^q$ with differential $d_V\otimes 1+1\otimes d_W$. %because it needs to be of degree 1$ %1 in the tensor means identity %talk about why this differential, maybe mention leibniz rule
...
IF THERE IS TIME MOTIVATE WITH SOMEE PICTURES IN TOPOLOGY
\item[$\ast$]<3-> When such a homotopy exists, $f$ and $g$ are \textbf{homotopy equivalent} or \textbf{homotopic}.
\item[$\ast$]<3-> Homotopy equivalence is an equivalence relation.
...
\begin{frame}
	...
	PICTURE OF THE INSERTION, I HAVE IT IN SOME OTHER SLIDES
\end{frame}
\begin{frame}
	\begin{defi}
...
\begin{frame}
\frametitle{Operadic suspension}
RELATION TO SHIFTT OF GRADED THINGS (I HAVE IT SOMEWHERE ELSE)
\begin{itemize}
