...
\begin{defin}
A symmetric monoidal category $\CC$ is \emph{closed} if for every object $A\in\CC$ the tensor product functor $A\otimes -:\CC\to\CC$ has a right adjoint functor $[A,-]:\CC\to\CC$. In other words, for all $A,B$ and $C\in \CC$ we have a natural bijection between the morphism sets
...
\begin{defin}
Let $(\VV ,⊗, 1)$ be a symmetric monoidal category and $(\CC,⊗, 1)$ a monoidal category. The category $\CC$ is a \emph{monoidal category over $\VV$} if there is an \emph{external (tensor) product} $∗ :\VV × \CC → \CC$ satisfying the following natural isomorphisms.%\pagebreak
\begin{itemize}
...
Throughout the thesis we will also assume that there is a bifunctor $\uC(−,−) : \CC^{op} × \CC → \VV$ such that there are natural
bijections
\[\Hom_\CC(C ∗ X, Y ) \cong \Hom_\VV (C,\uC(X, Y )).\]
Under the given assumption, we obtain a $\VV$-enriched category $\uC$ with the same objects as $\CC$, and the hom-objects are given by $\uC (−,−)$. The unit morphism $u_X : 1 → \uC (X,X)$ corresponds to the identity map in $\CC$ under the adjunction. The composition morphism is determined by the adjoint of the composite
...
that respects the associativity and unital structures of the monoidal categories over $\VV$ of $\CC$ and $\DD$. The coherence axioms for $ν_F$ are detailed in \cite[Proposition 10.1.5]{riehl}. If $ν_F$ is a natural isomorphism, then $F$ is said to be a \emph{functor over $\VV$}.
\end{defin}
\begin{defin}
Let $F,G : \CC → \DD$ be lax functors over $\VV$. A \emph{natural transformation over $\VV$} is a natural transformation
$μ : F ⇒ G$ such that for any $C ∈ \VV$ and for any $X ∈ \CC$ the following equality holds
...
\[μ : F(−) ⊗ F(−) ⇒ F(− ⊗ −)\]
is a natural transformation over $\VV$ that satisfies the standard unit and associativity conditions. If $ν_F$
and $μ$ are both natural isomorphisms then $F$ is said to be \emph{monoidal over $\VV$}. 
\end{defin}
...
If $F,G : \CC → \DD$ are lax functors over $\VV$, then $F$ and $G$ extend to $\VV$-enriched
functors
\[\underline{F},\underline{G} : \uC → \uD\]
where $\uC$ and $\uD$ denote the $\VV$-enriched categories corresponding to $\CC$ and $\DD$ as described in \Cref{underline}. Furthermore, any natural transformation $μ : F ⇒ G$ over $\VV$ also extends to a $\VV$-enriched natural
transformation
\[\underline{μ} : \underline{F} ⇒ \underline{G}.\]
In particular, if $F$ is lax monoidal over $\VV$,  then $\underline{F}$ is lax monoidal with respecto to the monoidal structure on $\uC × \uC$ described in \Cref{underline}.
\end{propo}
...
\begin{remark}
In the above proof, the fact that $\underline{\mu}$ is induced by $\mu$ is only essential when finding the inverse of the map $\uD(GX,Y)\to\uD(FX,Y)$, but similar constructions can be made for more general $\VV$-enriched natural transformation.
\end{remark}
...
These two concatenations are homotopic via reparametrization. Denote a homotopy by
\[
...
Our base category is the category of $\Z$-graded $R$-modules and linear maps, where $R$ is a commutative ring with unit of characteristic distinct from 2, so that the sign calculations are not trivial. All tensor products are taken over $R$. We denote the $i$-th degree component of $A$ as $A^i$. If $x\in A^i$ we write $\deg(x)=i$ and we use cohomological grading. The symmetry isomorphism is given by the following Koszul sign convention.
...
\item there is an $\infty$-morphism of $A_\infty$-algebras $f : H^*(A) \to A$ such that $f_1$ is a quasi-isomorphism lifting the identity.
\end{itemize}
...
\begin{defin}
Let $\OO$ be a collection. A \emph{cooperad} is a structure of comonoid on $\OO$ in the monoidal category $(\col, \circ, I)$, where $\circ$ is the plethysm defined in \Cref{plethysm} and  $I$ is the collection such that $I(0)=R$ and is trivial elsewhere.
...
%
%A \emph{morphism of filtered modules} is a morphism $f : A \to B$ of $R$-modules that preserves filtrations, namely 
%\[f(F_pA) ⊂ F_pB \text{ for all }p ∈ \Z.\]
%\end{defin}
...
We use the notation $\bgmod$ to represent the category with objects being bigraded $R$-modules and morphisms being morphisms of bigraded $R$-modules of bidegree $(0, 0)$. This category is equipped with a symmetric monoidal structure defined by the tensor product described above.
...
Let $(A, d^A_m)$ and $(B, d^B_m)$ be twisted complexes. A \emph{morphism of twisted complexes} $f : (A, d^A_m) → (B, d^B_m)$ is given by a family of morphisms of $R$-modules $\{f_m : A → B\}_{m≥0}$ of bidegree $(m,−m)$. This family must satisfy, for all $m ≥ 0$, the compatibility condition
\[\sum_{i+j=m}d^B_if_j =\sum_{i+j=m}(−1)^if_id^A_j.\]
The composition of such morphisms is given by $(g \circ f)_m :=\sum_{i+j=m} g_if_j$.
...
For a twisted complex $(A, d_m)$, we can define a map $d : \Tot(A) → \Tot(A)$ of degree $1$ by setting \[d(x)_j \coloneqq \sum_{m≥0}(-1)^{mn}d_m(x_{j-m})\] for $x = (x_i)_{i∈\Z} ∈ \Tot(A)^n$. Here, $x_i ∈ A^{n-i}_i$ represents the $i$-th component of $x$, and $d(x)_j$ denotes the $j$-th component of $d(x)$. It is important to note that, for any given $j ∈ \Z$, there exists a sufficiently large $m ≥ 0$ such that $x_{j-m′} = 0$ for all $m′ ≥ m$. Consequently, $d(x)_j$ is expressed as a finite sum. Moreover, for sufficiently negative $j$, one observes that $x_{j-m} = 0$ for all $m ≥ 0$, leading to $d(x)_j = 0$.
...
\begin{thm}
The assignments $(A, d_m) \mapsto (\Tot(A), d, F)$, where $F$ denotes the column filtration of $\Tot(A)$,
and $f \mapsto \Tot(f)$ establish a functor $\Tot : \tc \to \fc$. This functor restricts to an isomorphism onto its image.
\end{thm}
...
where $x ∈ A^{n-i}_i$ and $d(x)_k$ denotes the $k$-th component of $d(x)$. Note that $d(x)_k$ lies in $A^{n+1-k}_k$. We can also recover the bidegree of an element because in totalization they are sorted by horizontal degree and the non-negative horizontal degree components belong to the direct product factor, so we can locate the zero horizontal degree component if we represent the elements accordingly.
...
A justification for the above definition can be found in \cite[Lemma 4.18]{whitehouse}.
...
%\end{align*}
%where $v$ is the vertical degree of $f$. Furthermore, $f ∈ \ubgMod(A,B)$ is a morphism of twisted complexes if and
...
The monoidal on $\utC$ is given by the following map of vertical bicomplexes.
...
\end{align*}
the last map being a projection followed by a multiplication by the indicated sign factor.
\end{defin} 
...
f &\mapsto \Tot(f)
\end{align*}
define a $\vbc$-enriched functor $\mathfrak{Tot} : \utC → \ufC$ which restricts to an isomorphism onto its image. Moreover, this functor, when restricted further, becomes a $\bgmod$-enriched functor
\[\mathfrak{Tot} : \ubgMod → \ufMod,\]
also restricting to an isomorphism onto its image.
\end{thm}

We now present an enriched endomorphism operad. The precise operad structure is shown in \cite[Lemma 4.41]{whitehouse}. 
\begin{defin}
Let $\underline{\mathscr{C}}$ be a monoidal $\mathscr{V}$-enriched category and $A$ an object of $\uC$. We define the \emph{enriched endomorphism operad} $\uEnd_A$
...
are lax symmetric monoidal in the enriched sense and when restricted to the bounded case they become strong symmetric monoidal in the enriched sense.
...
$F : \CC → \DD$ be a lax monoidal functor over $\VV$. Then for any $X ∈ \CC$ there is a morphism of operads
\[\uEnd_X→\uEnd_{F(X)}.\]
