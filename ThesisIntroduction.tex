...
$X$ is thus a continuous map $f : S^ 1 \to X$ taking the base point of the circle to the base point $*$. We have the composition map

\[
m_2 : X ×X \to X
\]
sending a pair of loops $(f_1, f_2)$ to the loop $f_1 * f_2 = m_2(f_1, f_2)$ obtained by running through $f_1$ on the first half of the circle and through $f_2$ on the second half.
...
PICTURE OF NON ASSOCIATIVITY

Clearly, there is a homotopy
\[
m_3 : [0, 1]×X × X \to X
\]
joining the two possibilities of composing three loops by a reparamerization. When we want to compose 4
factors, there are 5 possibilities corresponding to the 5 binary trees with 4 leaves. Using
$m_3$, we obtain two paths of homotopies joining the composition $(f_1, f_2, f_3, f_4) \mapsto
((f_1 * f_2) * f_3) * f_4$ to the composition $(f_1, f_2, f_3, f_4) \mapsto f_1 * (f_2 * (f_3 * f_4))$.
...
These two paths are clearly homotopic. Let us denote an homotopy by
\[
m_4 : K_4 × (\Omega X)^4 \to X
\]
where $K_4$ denotes the pentagon bounded by the two paths. When we want to compose 5 factors, there are 14 possibilities corresponding to the 14 binary trees
with 5 leaves. Using $m_4$ and $m_3$, we obtain paths linking the compositions and faces linking the paths. The resulting sphere is the boundary of the polytope $K_5$ depicted.

PICTURE OF POLYTOPE (DON'T INCLUDE IMAGES FILES, DRAW IT IF NECESSARY)
...
of an $A_\infty$-algebra \cite{STASHEFF}.

\subsection{Definitions}

In this section we first establish notation and assumptions about graded modules. We then briefly recall some of the basic definitions regarding $A_\infty$-algebras.
...
Recall that if $(A,\partial^A)$ and $(B,\partial^B)$ are (co)chain complexes, the module $\Hom_R(A,B)$ also becomes a (co)chain complexes with differential
...
The above equation will sometimes be referred to as the $A_\infty$-\emph{equation}. This seemingly obscure definition captures the idea of an algebra that is associative up to homotopy. To see this, let us have a look at the first few cases.
...
Similarly to the $A_\infty$-equation (\cref{ainftyequation}), the above definition captures the idea of a map that is a morphism of algebras up to homotopy. This can be observed in the first few cases.
...
Over a field, one can replace any $A_\infty$-algebra by a quasi-isomorphic minimal one, where by quasi-isomorphic we mean that there is an map that induces an isomorphism on cohomology. This gives a very convenient way to describe a quasi-isomorphism class of an $A_\infty$-algebra. More precisely we have the following.

\begin{thm}[Kadeishvili] Let $A$ be a differential graded algebra over a field $k$, and let $H^*(A)$ be its cohomology module. Then $H^*(A)$ has an $A_\infty$-structure such that
\begin{itemize}
...
tUsing this result it is also possible to show that under some conditions any other dga $A'$ with $H^*(A')\cong H^*(A)$ is quasi-isomorphic to $A$. For more details see \cite{kade}.
\section{Operads}
...
\begin{defin}
The \emph{$\mathcal{A}_\infty$-operad} is the non-symmetric operad whose algebras are $A_\infty$-algebras, see \Cref{ainftyalgebra}. Therefore, it is generated by elements $\mu_i\in\mathcal{A}_\infty(i)$ satisfying the operadic version of the $A_\infty$-equation, \Cref{ainftyequation}.
...
I CAN ADD SOME EXAMPLE OF OPERAD AND A BIT MORE TEXT
...
the integers such that $F_{p}A ⊆ F_{p-1}A$ for all $p ∈ \Z$ and $A = \bigcup_p F_pA$. A \emph{morphism of filtered modules} is a morphism $f : A \to B$ of $R$-modules which is compatible with filtrations: 
\[f(F_pA) ⊂ F_pB \text{ for all }p ∈ \Z.\]
\end{defin}
We denote by $\mathrm{C}_R$ the category of chain complexes of $R$-modules.
...
We collect some definitions of basic bigraded categories that we need to use and establish some conventions.
...
bigraded $R$-modules. Indeed, an inverse of $f$ is obtained from an inverse of $f_0$ by solving a triangular system of linear equations.
\end{defin}
...
Here we recall the definition of the totalization functor from \cite{whitehouse} and some of the structure that it comes with. This functor and its enriched versions are key to establish a correspondance between $A_\infty$-algebras and derived $A_\infty$-algebras.
...
\end{align*}
Here the bidegree of $f$ is $(u, v)$. Furthermore, $f ∈ \ubgMod(A,B)$ is a map of twisted complexes if and
