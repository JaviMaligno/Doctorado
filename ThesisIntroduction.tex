...
\begin{defin}
Let $(\VV ,⊗, 1)$ be a symmetric monoidal category and let $(\CC,⊗, 1)$ be a monoidal category. We say that $\CC$ is a \emph{monoidal category over $\VV$} if we have an \emph{external tensor product} $∗ :\VV × \CC → \CC$ such that we have the following natural isomorphisms.%\pagebreak
\begin{itemize}
...
\begin{defin}
For a graded $R$-module $A$, the \emph{shift} or \emph{suspension} $SA$ is given by $SA^i=A^{i-1}$. %I think it is not wortth defining it on maps because I don't use it
\end{defin}
\pagebreak
...
A \emph{morphism} of $A_\infty$-algebras is a map $f:A\to B$ such that
...
A \emph{filtered complex} $(K, d, F)$ is a cochain complex $(K, d) ∈ \mathrm{C}_R$ together with a filtration $F$ of each $R$-module $K^n$ such that $d(F_pK^n) ⊂ F_pK^{n+1}$ for all $p, n ∈ \Z$. A \emph{morphism} of filtered complex is given by
a morphism of complexes $f : K → L$ compatible with filtrations, namely \[f(F_pK) ⊂ F_pL\text{ for all }p ∈ \Z.\]
\end{defin}
...
with
 \[F_p(A ⊗ B) :=\bigoplus_{i+j=p}\Ima(F_iA ⊗ F_jB → A ⊗ B).\] %image so that we have have inclusions
...
We define here some useful enriched categories and results from \cite[\S 4.3 and 4.4]{whitehouse}. Each of these categories will be a piece in \Cref{whitehouse}, which establishes a connection between $A_\infty$-algebras and derived $A_\infty$-algebras. Some of them have been modified according to our conventions. 
...
The following contains Proposition 4.40, Lemma 4.43 and Proposition 4.46 from \cite[\S 4.3]{whitehouse}.\pagebreak
