...
Finally, we prove our main result, Theorem \ref{theorem}, which relates the $A_\infty$-structure on an operad with the one induced on its Hochschild complex, more precisely, there is a morphism of $A_\infty$-algebras $\Phi:S\s\OO\to S\s\End_{S\s\OO}$. This result was hinted by Gerstenhaber and Voronov in \cite{GV}, but here we introduce more context and prove the theorem.
...
\section{$A_\infty$-algebra structure on an operad with $A_\infty$-multiplication}\label{sect2}


Let us consider the underlying graded module of the operad $\s\OO$, which we  call $\s\OO$ again by abuse of notation, i.e. \[\s\OO=\bigoplus_n \s\OO(n)\] with grading given by its natural degree, i.e. if $x\in \s\OO(n)$ recall that its natural degree $|x|=n+\deg(x)-1$ where $\deg(x)$ is its internal degree (the degree as an element of $\OO(n))$. 
...
\end{equation} 
Such $m$ is said to be an \emph{$A_\infty$-multiplication} on $\OO$ and as we saw in the previous text its existence is equivalent to a map of operads $\mathcal{A}_\infty\to \OO$ from the operad $\mathcal{A}_\infty$ of $A_\infty$-algebras to $\OO$. 
\end{defin}
...
1+\sum_i|x_i|.
\end{align*}
This means that the degree of the map $b_j(m_n;-)$ as map $\s\OO^{\otimes j}\to \s\OO$ does not depend on $n$ and equals $1$, so $|M'_j|=1$. In general, we have the following result.

\begin{lemma}
For $x\in\s\OO$ we have $|b_j(x;-)|=|x|$.
\end{lemma}
\begin{proof}
It follows by a calculation similar to the previous paragraph replacing $m_n$ by $x$. CONSIDER PLACING THIS BEFORE AND SHOWING MN AS A COROLLARY
...
\begin{propo}
AINFTY MULTIPLICATION ON O IMPLIES AINFTY ALGEBRA STRUCTURE ON SsO
\end{propo}
\begin{proof}
...
So now we have that $M\in\s\End_{S\s\OO}$ is a formal sum $M=M_1+M_2+\cdots$ such that $M_j$ has arity $j$ and internal degree $2-j$ (so that the natural degree is 1); and such that $M\tilde\circ M=0$. Therefore $M$ is the desired $A_\infty$-structure.
\end{proof}
...
\end{remark}
 ...
 A natural question to ask is what relation there is between the $A_\infty$-structure on $S\s\OO$ and the one on $S\s\End_{S\s\OO}$. In \cite{GV} it is claimed that given an operad $\OO$ with an $A_\infty$-multiplication, the map
...
holds, where the $M_j$ is the $A_\infty$-algebra structure map on $S\s\OO$ and the $\overline{M}_j$ is the $A_\infty$-algebra structure map on $S\s\End_{S\s\OO}$. THINK ABOUT HOW TO CALL THE STRUCTURE MAPS, MAYBE COMPONENTS, BUT SAY IN THE DEFINITION OF AINFTY MULTIPLICATION
\end{thm}
\begin{proof}
...
\end{align*}
where $l=n-(i_1+\cdots+i_j)+j$ and the sum with no subindex runs over all the possible order-preserving insertions. Note that $l\geq j$. Evaluating the above map on elements would yield Koszul signs coming from the brace relation. Also recall that $|b_j(x;-)|=|x|$. Now, fix some value of $l\geq j$ and let us compute
...
We are going to need to evaluate each of these terms omitting the sign $-(-1)^{|x|}$,  so let $z_1,\dots, z_n\in \s\OO$. We have by the brace relation that
...
where $\varepsilon$ is the usual Koszul sign with respect to the grading in $\s\OO$. We have to check that the insertions in $x$ that appear in $\mathcal{M}_1(\Phi'(x))$ (the right hand side of the equation \ref{case1}) are exactly those in the equation \ref{insertionx1} above (that correspond to the left hand side of equation \ref{case1}).
...
Each component can be evaluated on $(z_1,\dots, z_n)$ omitting the sign $-(-1)^{|x|}$ to produce
...
Note that we have to apply the Koszul sign rule twice: once at evaluation, and once more to apply the brace relation. The second sum in equationn \ref{insertionx2} is the same as the second sum in equation \ref{insertionx1}, so from now on we only need to use the first sum of the equation \ref{insertionx2}. Now, from the second summand of $\mathcal{M}_1(\Phi'(x))$ in the right hand side of equation \ref{mphi}
...
\end{align*}
 We are going to evaluate on $(z_1,\dots, z_n)$ to make this map more explicit (leaving out the sign factor $-(-1)^{|x|}$). This evaluation gives us the following
...
\section{Explicit $A_\infty$-structure maps}\label{sect4}
% PROBABLY ALSO INCLUDE THE COMPUTATION OF THE AINFTY EQUATION

We have given an implicit definition of the $A_\infty$-structure maps \[M_j=\overline{\sigma}(M'_j)=(-1)^{\binom{j}{2}}S\circ M'_j\circ(S^{-1})^{\otimes j},\]
...
For $x,x_1,\dots,x_n\in\s\OO$, the expression is the following
...
where $|x|$ is the degree of $x$ as an element of $\s\OO$. 
...
If we call $M^{GV}_2$ the product defined in \cite{GV} as \[M^{GV}_2=(-1)^{|x|+1}b_2(m;x,y),\] we see that $M_2=-M^{GV}_2$. Since they work in the associative case $m=m_2$, this minus sign does not affect the $A_\infty$-relation (which in this case reduces to the associativity and differential relations). This difference in sign can be explained by the difference between $(S^{\otimes n})^{-1}$ and $(S^{-1})^{\otimes n}$, since any of them can be used to define a map $(S\s\OO)^{\otimes n}\to \s\OO^{\otimes n}$. 
\end{remark}
%
