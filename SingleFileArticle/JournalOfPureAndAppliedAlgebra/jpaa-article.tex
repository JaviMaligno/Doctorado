...
\journalname{Journal of Pure and Applied Algebra}

%% Give the author list to appear in the running head
%% Example \runauth{Javier Aguilar Martín}
\runauth{}
...
%%
\title{The Derived Deligne Conjecture\tnoteref{label1}}
%% \tnotetext[label1]{}
\author{Javier Aguilar Martín\corref{cor1}\fnref{label2}}
\ead{ja683@kent.ac.uk;javiecija96@gmail.com}
%\ead[Linkedin profile]{https://www.linkedin.com/in/javier-aguilar-mart\%C3\%ADn-85575711a/}
%% \fntext[label2]{}
%% \cortext[cor1]{}
...
\address{School of Mathematics, Statistics and Actuarial Sciences, University of Kent at Canterbury, United Kingdom}
...
Operads\sep Derived $A_\infty-$algebras\sep Enriched categories\sep Deligne conjecture
%% MSC codes here, in the form: \MSC code \sep code
%% or \MSC[2008] code \sep code (2000 is the default)
\MSC[2020]{18M70}\sep \MSC[2020]{18M60}\sep \MSC[2020]{18N70}
\end{keyword}
...
\[(f\otimes g)(x\otimes y)=(-1)^{\deg(x)\deg(g)}f(x)\otimes g(y).\]  homomorphisms that raise the degree by i
...
objects in $\CC$. We denote by $\Hom_\CC(A,B)$ the set of morphisms from $A$ to $B$ in $\CC$. If $(\CC,\otimes, 1)$ is a
closed symmetric monoidal category, then we denote its internal hom-object by $[A,B] \in \CC$.
...
This makes the category of filtered $R$-modules into a symmetric monoidal category, where the unit is given by $R$ with the trivial filtration $0 = F_{1}R \subseteq F_0R = R$.
\end{defin}
...
to be the bigraded module of morphisms of bigraded modules $A \to B$. Furthermore, if $A,B$ are vertical bicomplexes, and $f \in
[A,B]^v_u$, we define
\[\delta(f) \coloneqq d_Bf -(-1)^vfd_A.\]
\end{defin}

\begin{lem}
If $A$, $B$ are vertical bicomplexes, then $([A,B]^*_*, \delta)$ is a vertical bicomplex.
\end{lem}
\begin{proof}
...
of morphisms $\{d_m : A \to A\}_{m\geq 0}$ of bidegree $(m,1-m )$ such that for all $m \geq 0$,
...
The category $(\tc,\otimes,R)$ is symmetric monoidal, where the monoidal structure is given by the bifunctor
\[\otimes : \tc \times \tc \to \tc.\]
On objects it is given by $((A, d^A_m), (B, d^B_m)) \to (A \otimes B, d^A_m \otimes 1 + 1 \otimes d^B_m)$ and on morphisms it is
given by $(f, g) \to f \otimes g$, where $(f \otimes g)_m :=\sum_{i+j=m} f_i \otimes g_j$. In particular, by the Koszul sign rule we
have that \[(f_i \otimes g_j)(x\otimes z) = (-1)^{\langle g_j ,x\rangle}f_i(x)\otimes g_j(z).\] The symmetry isomorphism is given by the strict
...
Given a twisted complex $(A, d_m)$, define a map $d : \Tot(A) \to \Tot(A)$ of degree $1$ by letting
...
that, for a given $j \in \Z$ there is a sufficiently large $m \geq 0$ such that $x_{j-m'} = 0$ for all $m' \geq m$. Hence
$d(x)_j$ is given by a finite sum. Also, for negative $j$ sufficiently large, one has $x_{j-m} = 0$ for all $m \geq 0$, which
implies $d(x)_j = 0$.

\begin{defin}
Given a morphism $f : (A, d_m) \to (B, d_m)$ of twisted complexes, we define the \emph{totalization of $f$} to be the map $\Tot(f) : \Tot(A) \to \Tot(B)$ of degree 0 defined by
\[(\Tot(f)(x))_j \coloneqq \sum_{m\geq 0}(-1)^{mn}f_m(x_{j-m})\]
 for $x = (x_i)_{i\in\Z} \in \Tot(A)^n$. 
 \end{defin}
...
When restricted to the bounded case, $\Tot : \bgmod^b
 \to \fmod^b$ and $\Tot : \tc^b \to \fc^b$ are
...
morphism $u_A : 1 \to \uC (A,A)$ corresponds to the identity map in $\CC$ under the adjunction, and the
...
and $\mu$ are natural isomorphisms then we say that $F$ is \emph{monoidal over $\VV$}. 
\end{defin}
...
\[\underline{\mathpzc{bgMod}_R}(A,B)^v_u \coloneqq \prod_{j\geq 0}[A,B]^{v-j}_{u+j}\]
...
\[(f, g) \to (f\underline{\otimes}g)_m :=\sum_{i+j=m}(-1)^{ij}f_i \otimes g_j\]
...
\[(\Tot(f)(x)))_{j+u} :=
\sum_{m\geq 0}(-1)^{(m+u)n}f_m(x_{j-m}) \in B^{n-j+v}_{j+u} \subseteq  \Tot(B)^{n+u+v}.\]
...
\begin{align*}
f_i : A^{m-j}_j \hookrightarrow\prod_{k\geq j}A^{m-k}_k &= F_j(\Tot(A)^m)\xrightarrow{g}F_{j+u}(\Tot(B)^{m+u+v})=\prod_{l\geq j+u}B^{m+u+v-l}_l\xrightarrow{\times(-1)^{(i+u)m}} B^{m-j+v-i}_{j+u+i} ,
\end{align*}
...
\[
\begin{tikzcd}
\ufC(\Tot(A)^{\otimes n},\Tot(A))\cong R\otimes\ufC(\Tot(A)^{\otimes n},\Tot(A))\arrow[r,"\underline{\mu}^{-1}\otimes 1"]&\ufC(\Tot(A^{\otimes n}),\Tot(A)^{\otimes n})\otimes\ufC(\Tot(A)^{\otimes n},\Tot(A))\arrow[d,"c"]\\ 
&\ufC(\Tot(A^{\otimes n}),\Tot(A)), 
\end{tikzcd}
\]
...
\end{equation}
This isomorphism can also be proved using the isomorphism $\s\s^{-1}\OO\cong\OO$ from \Cref{suspiso}, namely, we have $\s\End_{SA}\cong \s\s^{-1}\End_A\cong \End_A.$
...
\begin{proof}
The assignment preserves composition of maps. Indeed, given $g:\mathcal{P}\to\CC$, by definition \[\s(g\circ f)(x\otimes e^n)=g(f(x))\otimes e^n =\s g (f(x)\otimes e^n)=(\s g\circ \s f)(x\otimes e^n)\] This means that $\s$ defines an endofunctor on the category $\mathrm{Col}$ of collections.
...
\[\sigma=N\deg(g)(\deg(x)+N-1)+\deg(g)\sum_{j=1}^N(\deg(x_j)+a_j-1)(N-j)+\]\[\sum_{j<l}a_j(\deg(x_j)+\deg(g))+\sum_{j=1}^N(a_j+\deg(x_j)+\deg(g)-1)(N-j).\]
 ...
 \begin{align*}
 \deg(g)\sum_{j=1}^N(a_j-1)(N-j)+\sum_{j<l}a_j\deg(g)+\sum_{j=1}^N\deg(g)(N-j)&=\deg(g)\sum_{j=1}^Na_j(N-j)+\deg(g)\sum_{j<l}a_j\\
...
\[(N-1)\sum_{i=1}^nq_i+\sum_{i=1}^n (a_i-1)\sum_{l>i}q_l=\left(\sum_{j=0}^nk_j +n-1\right)\sum_{i=1}^nq_i+\sum_{i=1}^n (a_i-1)\sum_{l>i}q_l.\]
...
As we mentioned before, we can show an alternative description of $\infty$-morphisms of $A_\infty$-algebras and their composition in terms of suspension of collections, recall \Cref{inftymorphism} for the definition of these morphisms. Defining the suspension $\mathfrak{s}$ at the level of collections as we did in \Cref{functorial} allows us to talk about $\infty$-morphisms of $A_\infty$-algebras in this setting, since they live in collections of the form $\End^A_B=\{\Hom(A^{\otimes n},B)\}_{n\geq 1}$. More precisely, there is a left module structure on $\End^A_B$ over the operad $\End_B$
...
\[B_j(f;f_1,\dots, f_j)=\sum_{k_0+\cdots+k_j=n-j} f(1^{\otimes k_0}\otimes f_1\otimes 1^{\otimes k_1}\otimes\cdots\otimes f_j\otimes 1^{\otimes k_j}).\]
 The $1$'s in the braces are identity maps. In the above definition, $|f|$ denotes the degree of $f$ as an element of $\End_{\s\OO}$, which is the same as the degree $\overline{\sigma}(f)\in \s\End_{S\s\OO}$ because $\overline{\sigma}$ is an isomorphism, as mentioned in \Cref{barsigma}.  
...
\begin{align*}
Sb_n(&S^{-1}M_2(Sx_1,Sx_2);y_1,\dots, y_n) = \sum_{k=0}^n (-1)^{(|x_2|+1)\sum_{i=1}^k|y_i|}M_2(b_k(x_1;y_1,\dots, y_k),b_{n-k}(x_2;y_{k+1},\dots, y_n))
\end{align*}
and
\begin{align*}
...
\begin{align*}
(-1)^{\sum_{i=1}^n(n-i)|y_i|}Sb_n(S^{-1}M_1(Sx);y_1,\dots, y_n)=&\underset{\mathclap{1\leq i_1\leq n-k+1}}{\sum_{\mathclap{l+k-1=n}}}(-1)^{\varepsilon}M_l(Sy_1,\dots, Sb_{k}(x;y_{i_1},\dots),\dots, Sy_n)\\
...
In the following definition we use the notation in \cite{RW}.
  \begin{defin}
  A {derived $A_\infty$-algebra} on a $(\Z,\Z)$-bigraded $R$-module $A$ consist of a family of $R$-linear maps 
...
\begin{defin}
A \emph{morphism of filtered $A_\infty$-algebras} $f : A \to B$ from $(A,m_i, F)$ to $(B,m_i, F)$ is an $\infty$-morphism of $A_\infty$-algebras such that each map $f_j : A^{\otimes j} \to A$ is compatible with filtrations, i.e.
\[f_j(F_{p_1}A^{n_1} \otimes  \cdots \otimes  F_{p_j}A^{n_j} ) \subseteq  F_{p_1+\cdots +p_j}B^{n_1+\cdots +n_j+1-j} ,\]
for all $j \geq 1$, $p_1,\dots p_j \in \Z$ and $n_1,\dots , n_j \geq 0$.
\end{defin}
...
for $x=(x_i)_i\in \Tot(\OO(n))^{d_1}$ and $y=(y_j)_j\in \Tot(\OO(m))^{d_2}$. More generally, operadic composition $\bar{\gamma}$ is defined by the composite
\[
\begin{tikzcd}[row sep = 1.5em]
\Tot(\OO(N))\otimes \Tot(\OO(a_1))\otimes\cdots\otimes \Tot(\OO(a_N))\arrow[r, "\mu"]&\Tot(\OO(N)\otimes \OO(a_1)\otimes\cdots\otimes \OO(a_N))\arrow[r, "\Tot(\gamma)"]& \Tot\left(\OO\left(\sum a_i\right)\right),
\end{tikzcd}
\]
...
\end{equation*}
for $x=(x_k)_k\in\Tot(\OO(N))^{d_0}$ and $x^i=(x^i_k)_k\in\Tot(\OO(a_i))^{d_i}$, where $\varepsilon=\sum_{j=1}^m d_j\sum_{i=0}^{j-1}k_i$
...
On a bigraded operad we can use operadic suspension on the vertical degree with analogue results to those of the graded case that we explored in \Cref{Sec2}. We define $\Lambda(n)=S^{n-1}R$, where $S$ is a vertical shift of degree so that $\Lambda(n)$ is the underlying ring $R$ concentrated in bidegree  $(0,n-1)$. As in the single graded case, we express the basis element of $\Lambda(n)$ as $e^n=e_1\land\cdots\land e_n$. The operad structure on the bigraded $\Lambda=\{\Lambda(n)\}_{n\geq 0}$ is the same as in the graded case, see \Cref{lambdainsertion}.

\begin{defin}
Let $\mathcal{O}$ be a bigraded operad. The \emph{vertical operadic suspension} $\mathfrak{s}\OO$ is given by $\mathfrak{s}\OO(n)=\mathcal{O}(n)\otimes\Lambda(n)$ with diagonal composition. Similarly, we define the \emph{vertical operadic desuspension} $\mathfrak{s}^{-1}\OO(n)=\mathcal{O}(n)\otimes\Lambda^-(n)$.
\end{defin}
...
b_j^\star(x;x^1,\dots, x^N)_k=\smashoperator[r]{\sum_{\substack{k_0+k_1+\cdots+k_N=k\\ h_0+h_1+\cdots+h_N=j-N}}}(-1)^{\eta+\sum_{j=1}^m d_j\sum_{i=0}^{j-1}k_i}\gamma(x_{k_0};1^{h_0},x^1_{k_1},1^{h_1},\dots, x^N_{k_N},1^{h_N})
\end{equation}
...
\begin{defin}
We define the \emph{Hochschild cochain complex} of a bigraded module $A$ to be the bigraded module $S\s\End_A$. If $(A,d)$ is a vertical bicomplex, the Hochschild complex has a vertical differential $\partial(f)=[d,f]=d\circ f-(-1)^{q}f\circ d$, where $f$ has natural bidigree $(p,q)$ and $\circ$ is the plethysm corresponding to operadic insertions.
\end{defin}
In particular, $S\s\End_{S\s\OO}$ is the Hochschild cochain complex of $S\s\OO$. If $\OO$ has a derived $A_\infty$-multiplication, then the differential of the Hochschild complex $S\s\End_{S\s\OO}$ is given by $\overline{M}_{01}$ from \Cref{barbimaps}. The following is the same as \Cref{theorem} but carrying the extra index $i$ and using the bigraded sign conventions.
...
\begin{align*}
(-1)^{\sum_{i=1}^n(n-v)\mathrm{vdeg}(y_v)}Sb_n(S^{-1}M_{i1}(Sx);y_1,\dots, y_n)=&\underset{\mathclap{1\leq i_1\leq n-k+1}}{\sum_{\mathclap{l+k-1=n}}}(-1)^{\varepsilon}M_{il}(Sy_1,\dots, Sb_{k}(x;y_{i_1},\dots),\dots, Sy_n)\\
...
and
\begin{align*}
\eta=& \sum_{v=1}^{i_1-1}(k-v)\vdeg(y_v)+l\sum_{v=1}^{i_1-1}\vdeg(y_v)+\sum_{v=i_1}^{i_1+l-1}(k-i_1)\vdeg(y_v)+\sum_{v=i_1}^{n-l}(k-v)\vdeg(y_{v+l})
\end{align*}
...
Let us review the structure on the Hochschild complex of an associative operad in order to understand the question that we will be asking about the derived $A_\infty$-case. Let $\OO$ be an operad with an associative multiplication $m$, i.e. an $A_\infty$-multiplication $m$ such that $m=m_2$, see \Cref{ainftymult}. In this case, as a consequence of \Cref{ainftystructure} or by \cite[Proposition 2]{GV}, we have a dg-algebra structure on $S\s\OO$ given by the differential
...
\end{align*}
 and we denote the exponent by $\varepsilon=\sum_{j=1}^n (a_j+q_j-1)(n-j+\sum_j^n k_l)$. So now we have that, decomposing $S^{\otimes N-n+\sum a_j}$, the last map up to multiplication by $(-1)^{\binom{N-n+\sum a_j}{2}+\varepsilon}$ is
