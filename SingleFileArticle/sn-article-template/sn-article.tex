...
\[?_{A?B} : A ? B ? B ? A,\ x ? y \mapsto (?1)^{\langle x,y\rangle}y ? x.\]
...
\begin{lem}
If $A$, $B$ are vertical bicomplexes, then $([A,B]^?_?, ?)$ is a vertical bicomplex.
\end{lem}
\begin{proof}
...
by the bifunctor
\[\otimes : \tc \times \tc ? \tc.\]
On objects it is given by $((A, d^A_m), (B, d^B_m)) ? (A \otimes  B, d^A_m \otimes  1 + 1 \otimes  d^B_m)$ and on morphisms it is
given by $(f, g) ? f \otimes  g$, where $(f \otimes g)_m :=\sum_{i+j=m} f_i \otimes g_j$. In particular, by the Koszul sign rule we
...
\[
?_{A\otimes B} \colon A \otimes  B ? B \otimes  A,\ x \otimes  y\mapsto (?1)^{\langle x,y\rangle}y \otimes  x.
\]
\end{lem}
...
\[\epsilon : R ? \Tot(R)\text{ and }\mu=?_{A,B} : \Tot(A) \otimes  \Tot(B) ? \Tot(A \otimes  B)\]
given by $\epsilon = 1_R$. For $x = (x_i)_i ? \Tot(A)^{n_1}$ and  $y=(y_j)_j ? \Tot(B)^{n_2}$,
\begin{equation}\label{mu1}
?(x \otimes  y)_k \coloneqq
...
\begin{defin}
Let $(\VV ,\otimes , 1)$ be a symmetric monoidal category and let $(\CC,\otimes , 1)$ be a monoidal category. We say that $\CC$ is a \emph{monoidal category over $\VV$} if we have an external tensor product $? :\VV \times \CC ? \CC$ such that we have the following natural isomorphisms.
\begin{enumerate}[$\bullet$]
\item  $1 ? X \cong X$ for all $X ? \CC$,
\item $(C \otimes  D) ? X \cong C ? (D ? X)$ for all $C,D ? \VV$ and $X ? \CC$,
\item $C ? (X \otimes  Y ) \cong (C ? X) \otimes  Y \cong X \otimes  (C ? Y )$ for all $C ? \VV$ and $X, Y ? \CC$.
\end{enumerate}
\end{defin}
\begin{remark}\label{underline}
We will also assume that there is a bifunctor $\uC(?,?) : \CC^{op} \times   \CC ? \VV$ such that we have natural
bijections
...
enriched functor
\[\underline{\otimes } : \uC \times   \uC ? \uC\]
...
\[\uC (X,W) \otimes  \uC (Y,Z) ? \uC (X \otimes  Y,W \otimes  Z),\]
...
\begin{defin}
Let $\CC$ and $\DD$ be monoidal categories over $\VV$. A \emph{lax functor over $\VV$} consists of a functor $F : \CC ? \DD$ together with a natural transformation \[?_F : ? ?_\DD F(?) ? F(? ?_\CC ?)\]
which is associative and unital with respect to the monoidal structures over $\VV$ of $\CC$ and $\DD$. See \cite[Proposition 10.1.5]{riehl} for explicit diagrams stating the coherence axioms. If $?_F$ is a natural isomorphism
...
$? : F ? G$ such that for any $C ? \VV$ and for any $X ? \CC$ we have
\[?_G \circ (1 ?_\DD ?_X) = ?_{C?_\CC X} \circ ?_F .\]
A \emph{lax monoidal functor over $\VV$} is a triple $(F, \epsilon, ?)$, where $F : \CC ? \DD$ is a lax functor over $\VV$,
$\epsilon : 1_\DD ? F(1_\CC)$ is a morphism in $\DD$ and
\[? : F(?) \otimes  F(?) ? F(? \otimes  ?)\]
is a natural transformation over $\VV$ satisfying the standard unit and associativity conditions. If $?_F$
and $?$ are natural isomorphisms then we say that $F$ is \emph{monoidal over $\VV$}. 
\end{defin}
...
where $\uC$ and $\uD$ denote the $\VV$-enriched categories corresponding to $\CC$ and $\DD$ as described in \Cref{underline}. Moreover, any natural transformation $? : F ? G$ over $\VV$ also extends to a $\VV$-enriched natural
transformation
\[\underline{?} : \underline{F} ? \underline{G}.\]
...
\[c : \underline{\mathpzc{bgMod}_R}(B,C) \otimes  \underline{\mathpzc{bgMod}_R}(A,B) ? \underline{\mathpzc{bgMod}_R}(A,C)\]
by
...
\[?(f) := c(d^B, f) ? (?1)^{\langle f,d^A\rangle}c(f, d^A) ? \underline{\mathpzc{bgMod}_R}(A,B)^{v+1}_u.\]
More precisely,
\[(?(f))_m :=\sum_{i+j=m}(?1)^{i|f|}d^B_if_j ? (?1)^{v+i}f_id^A_j.\]
\end{defin}
...
&c(d^A, d^A) = 0\\
&?^2 = 0\\
&?(c(f, g)) = c(?(f), g) + (?1)^v c(f, ?(g))
\end{align*}
where $v$ is the vertical degree of $f$. Furthermore, $f ? \ubgMod(A,B)$ is a map of twisted complexes if and
only if $?(f) = 0$. In particular, $f$ is a morphism in $\tc$ if and only if the bidegree of $f$ is $(0, 0)$ and
$?(f) = 0$. Moreover, for $f$, $g$ morphisms in $\tc$, we have that $c(f, g) = f\circ g$, where the latter denotes
composition in $\tc$.
\end{lem}

\begin{defin}
For $A,B$ twisted complexes, we define $\underline{t\mathcal{C}_R}(A,B)$ to be the vertical bicomplex
$\underline{t\mathcal{C}_R}(A,B) := (\underline{\mathpzc{bgMod}_R}(A,B), ?)$.
\end{defin}
...
\item The composition morphism $c:\ubgMod(B,C) \otimes  \ubgMod(A,B) ? \ubgMod(A,C)$ is given by \Cref{weirdenrichment}.
...
\item The composition morphism $c : \utC(B,C)\otimes \utC(A,B) ? \utC(A,C)$ is given by \Cref{weirdenrichment}.
...
\[\underline{\otimes }: \utC(A,B) \otimes  \utC(A?,B?) ? \utC(A \otimes  A?,B \otimes  B?)\]
\[(f, g) ? (f\underline{\otimes }g)_m :=\sum_{i+j=m}(?1)^{ij}f_i \otimes  g_j\]
...
\[?(f) := c(d^L, f) ? (?1)^{\langle f,d^K\rangle}c(f, d^K) = d^Lf ? (?1)^{v+u}fd^K = d^Lf ? (?1)^{|f|}fd^K\]
...
\[\underline{\otimes }: \ufC(K,L) \otimes  \ufC(K?,L?) ? \ufC(K \otimes  K?,L \otimes  L?),\]
...
\[
\mathfrak{Tot}_{A,B} : \utC(A,B) ? \ufC(\Tot(A),\Tot(B)),\,  f \mapsto \Tot(f)
\]
...
\[\uEnd_A(n) \coloneqq \uC (A^{\otimes n},A) \text{ for }n ? 1.\]
\end{defin}
...
In the case that $f:A\to A$ is an $\infty$-endomorphism, \Cref{operadicinftymorphism} can be written in terms of operadic composition as $f\tilde{\circ}m=\tilde{\gamma}(m\circ f)$. 
...
\[m_i(F_{p_1}A^{n_1} \otimes  \cdots \otimes  F_{p_i}A^{n_i} ) ? F_{p_1+\cdots
...
$f : (A,m_i) ? (B,m_i)$ of $A_?$-algebras such that each map $f_j : A^{\otimes j} ? A$ is compatible with filtrations, i.e.
\[f_j(F_{p_1}A^{n_1} \otimes  \cdots \otimes  F_{p_j}A^{n_j} ) ? F_{p_1+\cdots +p_j}B^{n_1+\cdots +n_j+1?j} ,\]
...
denotes the subset of morphisms which send $?_{i1}$ to $d^A_i$. We view $\mathcal{A}_?$ as an operad in $\vbc$ sitting in
...
\begin{enumerate}[(1)]
\item A twisted complex $(A, d^A)$ together with a morphism $\calA_? ? \uEnd_A$ of operads in $\vbc$, which is determined by a family of elements $M_i ? \utC(A^{\otimes i},A)^{2?i}_0$ for $i ? 2$ for which the $A_\infty$-relations holds for $i\geq 2$, \Cref{ainftyequation}. The composition is the one prescribed by the composition morphisms of $\utC$.
\item A bigraded module $A$ together with a family of elements $M_i ? \ubgMod(A^{\otimes i},A)^{2?i}_0$ for $i ? 1$ for
