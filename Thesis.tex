...
There is a morphism of $A_\infty$-algebras $\Phi:S\s\OO\to S\s\End_{S\s\OO}$ lifting the canonical brace map $\s\OO\to \End_{\s\OO}$.
\end{manualtheorem} 
...
There is a morphism $\Phi:S\s\OO\to S\s\End_{S\s\OO}$ of derived $A_\infty$-algebras lifting the canonical brace map $\s\OO\to\End_{\s\OO}$.
\end{manualtheorem}
