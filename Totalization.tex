...
We collect some notions and results about enriched categories from RIEHL and WHITEHOUSE that we will need as a categorical setting for our results on derived $A_\infty$-algebras.
...
which is associative and unital with respect to the monoidal structures over $\VV$ of $\CC$ and $\DD$. (See [Rie14,
Proposition 10.1.5] REFERENCE) for explicit diagrams stating the coherence axioms.) If $ν_F$ is a natural isomorphism
...
\[
% https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZARgBoAGAXVJADcBDAGwFcYkQAdD5gEQAoA4gA1SATQCUXCHgC28AATEQAX1LpMufIRQAmCtTpNW7Lr0EiJU2fFP8AYiOHiVakBmx4CRcvpoMWbIic3PzCYuLyLuoeWkQAzL6GASYhfA7hUW4antokpMQG--Z8DoH4GsaZoWtoID-oBdorrGYp8vBoFBOBkEgChxDoV+WG-kEeFAdeaJirGpGIRByFTnw4p7JQyhAA
\begin{tikzcd}
{\uD(GX,Y) } \arrow[r, "\cong"] \arrow[rd, "(5)", dotted, bend left]                 & {\uD(GX,Y)\otimes 1} \arrow[r, "1\otimes\umu_X"] \arrow[d, "1\otimes\alpha_X"] \arrow[rd, "(4)", dotted] & {\uD(GX,Y)\otimes\uD(FX,GX)} \arrow[r, "c"] \arrow[d, "\cong"]                                                                     & {\uD(FX,Y)} \arrow[ddd, "\cong"]                                               \\
                                                                                     & {\uD(GX,Y)\otimes\uD(GX,GX)} \arrow[lu, "c"] \arrow[lu]                                                  & {\uD(GX,Y)\otimes\uD(FX,GX)\otimes 1} \arrow[ld, "1\otimes 1\otimes \umui_X"] \arrow[rdd, "c\otimes 1"] \arrow[ru, "(1)"', dotted] &                                                                                \\
                                                                                     & {\uD(GX,Y)\otimes\uD(FX,GX)\otimes \uD(GX,FX)} \arrow[u, "1\otimes c"] \arrow[ld, "c\otimes 1"]          &                                                                                                                                    &                                                                                \\
{\uD(FX,Y)\otimes\uD(GX,FX)} \arrow[uuu, "c"] \arrow[ruu, "(3)"', dotted, bend left] & {}                                                                                                       &                                                                                                                                    & {\uD(FX,Y)\otimes 1} \arrow[ll, "1\otimes\umui_X"] \arrow[llu, "(2)"', dotted]
\end{tikzcd}
\]
\end{proof}
\subsubsection{Enriched categories and totalization}

We define here some useful enriched categories and results from WHITEHOUSE. Some of them had to be modified to adjust them to our conventions. 
...
\[\uEnd_A →\uEnd_{\Tot(A)},\]
which is an isomorphism of operads if $A$ is bounded. The same holds true if $A$ is just a bigraded module.
\end{lem}
\begin{proof}
The proof of in the case of a $A$ beig a twisted complex can be found in LEMMA 4.54. For the bigraded module case, we are going to do it analogously. First, by THEOREM 4.39 we know that $\mathfrak{Tot}:\ubgMod\to\ufMod$ is $\bgmod$-enriched. In fact, by LEMMA 4.37 it is lax monoidal in the enriched sense. In addition, both $\ubgMod$ and $\ufMod$ are monoidal over $\bgmod$ by LEMMA 4.15. With all of this we may apply \Cref{morphism} to $\mathfrak{Tot}:\ubgMod\to\ufMod$ to obtain the desired map
\[\uEnd_A →\uEnd_{\Tot(A)}.\]
...
This map can be computed explicitly by iteration of the insertion $\bar{\circ}$. %For simplicity, we abuse of notation by omitting sums
...
\end{equation}
for $x=(x_k)_k\in\Tot(\OO(N))^{n_0}$ and $x^i=(x^i_k)_k\in\Tot(\OO(a_i))^{n_i}$. 
...
HEURISTICS FOR THIS SIGN AND ALTERNATIVE?
...
There exist other possible ways of totalizing operads by varying the natural transformation $\mu$. For instance, we can choose the totalization functor $\Tot'$ which is the same as $\Tot$ but  with $\mu$ defined in such a way that the insertion on $\Tot'\OO$ is defined by \[(a\hat{\circ}b)_k=\sum_{k_1+k_2=k}(-1)^{k_2n_1}a_{k_1}\circ b_{k_2}.\] 
...
It can be verified that $\Tot'(\s\OO)=\s \Tot'(\OO)$. With the original totalization we have a non identity isomorphism given by \Cref{extrasign}. %We will use this isomorphism later.
...
Similarly, a \emph{derived $A_\infty$ multiplication} on a bigraded operad $\OO$ is an element $m=\sum_{ij}m_{ij}$ where $m_{ij}\in\OO(j)^{2-i-j}_i$ for each $j\geq 1$ such that 
...
From the definition of $\Tot(\s\OO)$, a derived $A_\infty$-multiplication is equivalent to an element $m\in \Tot(\s\OO)$ of degree 1 concentrated in positive arity satisfying $m\star m=0$. From this, it follows from our previous results PREVIOUS ARTICLE that $m$ determines an $A_\infty$-algebra structre on $S\Tot(\s\OO)\cong S\s \Tot(\OO)$ %IT WAS EQUALITY WITH THE ALTERNATIVE OPERATION 
...
First recall the definition of a brace algebra and introduce the definition for the bigraded case.
...
The other factor signis  produced by totalization. This was computed in \Cref{totcomp}. Combining both factors we obtain the following.
...
\section{Derived $A_\infty$-structure on an operad}


We are going to use the following theorem from WHITEHOUSE (CITE) to show that there is a derived $A_\infty$-structure on $A=S\s\OO$. Note that $\Tot(SB)=S\Tot(B)$ for any bigraded module $B$, where $SB$ is the vertical suspension of $B$ and $S\Tot(B)$ is the suspension of $\Tot(B)$ as graded modules.
...
In this section we provide a description of the twisted complex structure on an operad $\OO$ with a derived $A_\infty$-multiplication.
