...
Let $K$ and $L$ be filtered complexes. We define $\underline{\Hom}(K,L)$ the filtered complex with differential SAME AS USUAL and filtration $F$ given by 
...
\end{lem}
\begin{proof}
The proof is included in the proof of LEMMA 4.35.
\end{proof}
...
\end{align*}
The map $\underline{\mu}^{-1}$ is the adjoint of $\mu^{-1}$ under the bijection from \Cref{adjunction}. Explicitly,
\begin{align*}
...
PROP 4.55
\end{proof}
I SHOULD DEFINE END-TOT AND SAY THAT ALTERNATIVE WE  CAN JUST USE THE ONES THAT PRESERVE FILTRATION BECAUSE THAT  IS PRECISELY THE IMAGE OF THE MAPS
\section{Operadic totalization and vertical operadic suspension}
...
\end{definition}
\begin{remark}
One can check that, on any derived $A_\infty$-algebra $A$, the maps $d_i=(-1)^{i}m_{i1}$ define a twisted complex structure. This leads to the possibility of defining a derived $A_\infty$-algebra as a twisted complex with some extra structure. SEE  REMARK 4.48 BELOW  SO LEAVE THIS AS AN EXAMPLE, AS WELL AS AINTTY ALGEBRAS (THAT WAY I CAN WRITE THE RELATIONS AND REFER TO THEM)
\end{remark}
...
\begin{remark}
THE OTHER POSIBILITY FOR DEFINING $\End_K$ COULD BE STATED HERE, MAYBE DENOTE $\End^0_K$ THE OTHER
...
morphisms of operads in filtered complexes $A_∞ → \End_K$. To see this, notice that if one forgets the
...
\end{remark}
DEF 4.53 (NOT SURE IF I NEED IT)
...
\[\Hom_{\mathrm{vbOp}}(\calA_∞, \uEnd_{\Tot(A)})\cong \Hom_{\mathrm{fCOp}}(\calA_∞,\End_{\Tot(A)})\]
...
I NEED TO MAKE CLEAR THAT THE DIFFERENTIAL ON ENDA IS GIVEN BY [D0,]
