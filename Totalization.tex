...
I GUESS I DON'T NEED TO WRITE IT AGAIN IF THIS IS A CONTINUATION
\begin{align*}
...
b_j^T(x;x^1,\dots, x^N)_k=\sum_{k_0+k_1+\cdots+k_N=k}(-1)^{\sum_{j=1}^m n_j\sum_{i=0}^{j-1}k_i+\eta}\gamma(x_{k_0};x^1_{k_1},\dots, x^N_{k_N})
\end{equation}
for $x=(x_k)_k\in\Tot(\s\OO(N))^{n_0}$ and $x^i=(x^i_k)_k\in\Tot(\s\OO(a_i))^{n_i}$. 
\end{lem}
