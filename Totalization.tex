...
Let $K$ and $L$ be filtered complexes. We define $\underline{\Hom}(K,L)$ the filtered complex whose underlying chain complex is $\Hom_{\mathrm{C}_R}(K,L)$ and the filtration $F$ given by 
...
NOT SURE IF I NEED TO STATE EVERYTHING THAT FOLLOWS FROM THIS

\begin{defin}
LAX FUNCTOR AND LAX MONOIDAL FUNCTOR
\end{defin}
...
\bar{\gamma}(x;x^1,\dots, x^N)_k=\sum_{k_0+k_1+\cdots+k_N=k}(-1)^{\sum_{j=1}^m n_j\sum_{i=0}^{j-1}k_i}\gamma(x_{k_0};x^1_{k_1},\dots, x^N_{k_N})
\end{equation}
...
where $a(h_p)$ is the arity of $h_p$ (in this case $h_p$ is either $1$ or some $g_i$). If we want to express this composition in terms of the composition in $\OO$ we just have to find out the factor sign applying the same strategy as in the single-graded case. In fact, there is a sign factor identical to the graded case replacing internal degree by vertical (internal) degree that comes from operadic suspension. This is the sign that determines the brace $b_j$ on $\s\OO$. The other factor signs are produced by totalization. This was computed in \Cref{totcomp}. 
...
MAYBE WRITE THIS AS A LEMMA LIKE I DID IN THE CLASSICAL CASE?
