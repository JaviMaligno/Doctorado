...
See THEOREM 3.8.
\end{proof}
For a filtered complex of the form $(\Tot(A),d,F)\in\sfc$, where $A = \{A^j_i \}$ is a bigraded $R$-module, we can recover the twisted complex structure on  $A$ as follows. For all $m ≥ 0$, let
...
\end{propo}
\begin{proof}
PROP 4.46 I WANT TO GIVE A MORE DETAILED PROOF DEFINING THE MAP FROM THE MONOIDAL THING
\end{proof}
...
We work our way backwards through the proof of WHITEHOUSE THEOREM. The isomorphism WRITE ISO (CHECK COMMANDS) does not modify the map $M_j$ at all. 

THE FINAL SIGN CHANGE COINCIDES WITH THE ISOMORPHISM COMMUTING SUSPESION AND TOT SO IN THE END IT CANCELS

We are going to split the isomorphism THE ENRICHED ONE in two parts. DO THIS IN A VERSION OF LEMMA 4.54

TOT-1 ALREADY SPLIT THINGS AND THEN IT IS THE FINAL CHANGE
%\appendix
