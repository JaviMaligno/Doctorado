...
We start with the $A_\infty$-structure on $S\s\Tot(\OO)$, which is equivalent to an $A_\infty$-structure on $S\Tot(\s\OO)$ via the isomorphism $S\s\Tot(\OO)\cong S\Tot(\s\OO)$ from \Cref{extrasign}. This isomorphism introduces some signs that will cancel with the sigs introduced by the isomorphism FIRST ISO OF THE CHAIN so we will omit them. The $A_\infty$-structuure is therefore given by maps THINK WHERE I HAVE TO PUT SUSPENSION
...
We can view $M_j$ as the image of a morphism of operads of filtered complexes $f:\mathcal{A}_\infty\to \End_{\Tot(S\s\OO)}$, in such a way that $M_j=f(\mu_j)$ for $\mu_j\in\mathcal{A}_\infty(j)$. 

We work our way backwards through the proof of WHITEHOUSE THEOREM. The isomorphism WRITE ISO (CHECK COMMANDS) does not modify the map $M_j$ at all, but allows us to see it as a element of $\uEnd_{\Tot(A)}$ of bidegree $(0,2-j)$. 



The isomorphism THE ENRICHED ONE in the direction we are followig has been described after \Cref{inverse}. Under this isomorphism, $f$ is sent firstly to the map \[\mu_j\mapsto \mathfrak{Tot}^{-1}\circ c(M_j,\mu^{-1})=\mathfrak{Tot}^{-1}\circ M_j\circ \mu^{-1},\] where $c$ is the composition in $\ufC$. The functor $\mathfrak{Tot}^{-1}$ decomposes $M_j$ into a sum of maps $M_j=\sum_i M_{ij}$. More explicitly, let $x_1\otimes\cdots\otimes x_j\in (A^{\otimes j})^{m-k}_k$. Then we have
\begin{align*}
...
&=\sum_i(-1)^{im}\sum_l Sb_j^T(m_{il};(S^{-1})^{\otimes j}(\mu^{-1}(x_1\otimes\cdots\otimes x_j)))\\
&=\sum_i(-1)^{im}\sum_l(-1)^{i(m-1)} Sb_j(m_{il};(S^{-1})^{\otimes j}(\mu^{-1}(x_1\otimes\cdots\otimes x_j)))\\
&=\sum_i\sum_l(-1)^{i} Sb_j(m_{il};(S^{-1})^{\otimes j}(\mu^{-1}(x_1\otimes\cdots\otimes x_j)))
\end{align*}
so that $M_{ij}(x_1,\dots,x_j)=\sum_l(-1)^{i} Sb_j(m_{il};(S^{-1})^{\otimes j}(\mu^{-1}(x_1\otimes\cdots\otimes x_j)))$, where $b_j$ is the brace on $A=S\s\OO$.



According to the isomorphism FIRST ISO this defines a derived $A_\infty$-structure on the twisted complex $S\s\OO$ up to a sign that cancels with the sign introduced by the iso from \Cref{extrasign}.

NOW WITH ALL THE SIGNS AND
%\appendix
