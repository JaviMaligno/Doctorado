...
\begin{frame}[fragile]
 We can take the nerve $N(PaB(n))$ and the geometric realization $|N(PaB(n))|$ to obtain a topological operad. \pause %SAY THIS IS A WAY OF EXTRACTING A SIMPLICIAL COMPLEX FROM A CATEGORY (VERTICES ARE OBJETS, EDGES ARE OBJECTS AND TRIANGLES BELOW. IF SOMEONE DOESN'T WHAT THIS IS, IT'S A WAY OF GLUING TRIANGLES TO MAKE SPACES\pause
 \begin{figure}
 $
 \begin{tikzcd}
 & z \\
...
\begin{frame}
	\begin{itemize}
		\item<1-> %SAY THIS COMES FROM A LINEAR VERSION OF THE NERVE %Taking a $\Q$-linear version of the nerve and constructing the associated chain complex we obtain 
		Define the chain operad $C_*(O_A)$ where $C_k(O_A(n))=A_n^{\otimes k}$ and
		$$
		d(a_1\otimes\cdots\otimes a_k)=\pi(a_1)a_2\otimes\cdots\otimes a_k-a_1a_2\otimes \cdots\otimes a_k+\cdots$$ $$\cdots+(-1)^{k}a_1\otimes\cdots\otimes a_{k-1}\pi(a_k).$$ %comentar que a esto se llega en varios pasos previos, tal como comento en el trabajo
		...
		\item<3-> The quasi-isomorphism $\phi:C_*(|N(PaB)|)\to C_*(O_A)$ is produced by the morphism $k(PaB)\to O_A$ which is trivial on objects and on morphisms is defined by $\phi(x)=\exp(t_{12}/2)$ and $\phi(i)=\Phi\in A_3$ is a \textbf{Drinfeld associator}.  %decir que O_A es esencialmente A vista como categoría Q-aditiva, es decir, los morfismos forman un Q-e.v.
