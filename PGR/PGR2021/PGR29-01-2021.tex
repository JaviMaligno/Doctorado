...
\title{$A_\infty$-algebras in an operadic context}
...
\item<2-> Homotopy: a \textbf{homotopy} between two maps $f,g:X\to Y$ is a continuous family of maps $h_s:X\to Y$ for $s\in[0,1]$ such that $h_0=f$ and $h_1=g$. We write $f\simeq g$. %maybe add some picture, one map is transformed onto the other
...
\begin{frame}
\frametitle{Intuition behind homology}
\begin{itemize}
\item<1-> The dimension of $H_0(X)$ is the number of connected components of $X$. %whole pieces of the space
\item<2-> $H_n(X)$ ($n\geq 1$) measures $n$-dimensional ``holes'' or ``cavities''.
\item<3-> For example, $\dim H_0(S^2)=1$ because it is connected and $\dim H_2(S^2)=1$ because there is a 2-dimensional cavity. %hollow sphere
\item[]<4-> $\dim H_n(S^2)=0$ for all $n\neq 0,2$ %there are no other holes or cavities
\end{itemize}
\end{frame}
\begin{frame}
\frametitle{How to detect holes}
\begin{itemize}
\item<1-> $d$ acts as a boundary operator %holes are things that look like boundaries but are not because they are not filled
\definecolor{uuuuuu}{rgb}{0.26666666666666666,0.26666666666666666,0.26666666666666666}
\begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1.0cm,y=1.0cm]
\clip(-0.1,-0.18) rectangle (12,3);
\fill[fill=black,fill opacity=0.10000000149011612] (0.,0.) -- (3.,0.) -- (1.5,2.5980762113533165) -- cycle;
\draw(5.,0.) -- (8.,0.) -- (6.5,2.5980762113533165) -- cycle;
\draw (0.,0.)-- (3.,0.);
\draw (3.,0.)-- (1.5,2.5980762113533165);
\draw (1.5,2.5980762113533165)-- (0.,0.);
\draw (5.,0.)-- (8.,0.);
\draw (8.,0.)-- (6.5,2.5980762113533165);
\draw (6.5,2.5980762113533165)-- (5.,0.);
\draw [->] (3.,1.) -- (5.,1.);
\draw [->] (8.,1.) -- (10.,1.);
\draw (3.8,1.7) node[anchor=north west] {$d$};
\draw (8.736875000000042,1.7) node[anchor=north west] {$d$};
\draw (10.25,1.31250000000047) node[anchor=north west] {\Large{$0$}};
%\begin{scriptsize}
%\draw [fill=black] (0.,0.) circle (1.5pt);
%\draw [fill=black] (3.,0.) circle (1.5pt);
%\draw [fill=uuuuuu] (1.5,2.5980762113533165) circle (1.5pt);
%\draw [fill=black] (5.,0.) circle (1.5pt);
%\draw [fill=black] (8.,0.) circle (1.5pt);
%\draw [fill=uuuuuu] (6.5,2.5980762113533165) circle (2.5pt);
%\end{scriptsize}
\end{tikzpicture}
  \item<2-> Since the hollow triangle is the boundary of a filled triangle, it becomes 0 in homology.
  \item<3-> If we only had the hollow triangle it would be a generator of homology.
\end{itemize}
...
\begin{itemize}
\item<1-> The maps $M_i:K_i\times X^i\to X$ induce maps $C_*(X)^{\otimes i}\to C_*(X)$. %esentially the induced map on chains but with extra steps
...
\frametitle{Operads}
	\begin{itemize}
			\item<1-> An \textbf{operad} can be intuitively thought as a collection of spaces  $\mathcal{O}=\{\mathcal{O}(n)\}_{n\geq 0}$, whose points are thought to be $n$-ary operations $X^n\to X$. %decir que pueden ser (topological spaces, vector spaces, other objects) siempre que los axiomas tengan sentido, es decir, comentar que esto se puede hacer en cualquier categoría monoidal simétrica diciendo por encima lo que es: producto, unidad y axiomas. Poner algo de todos modos en alguna diapositiva
...
\begin{frame}
	\begin{itemize}
		\item There are \textbf{composition maps} $\gamma : \mathcal{O}(n) \times \mathcal{O}(j_1) \times \cdots \times \mathcal{O}(j_n) \to \mathcal{O}(\sum_s j_s)$
...
%	\begin{block}{}
%	\begin{itemize}
%	\item An identity element $1 \in \mathcal{O}(1)$ such that 
%	$\gamma(1; d) = d$ for $d \in \mathcal{O}(j)$ and 
%	$\gamma(c; 1,\dots,1) = c$ for
%	$c \in \mathcal{O}(k)$.
%	
%	\item A right action of the symmetric group $\Sigma_j$ on $\mathcal{O}(j)$ such that the following equivariance
%	formulas are satisfied for all $c\in \mathcal{O}(k)$, $d_s \in \mathcal{O}(j_s)$, $\sigma\in\Sigma_k$, and $\tau_s\in\Sigma_{j_s}$:
%	\[
