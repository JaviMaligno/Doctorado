...
There are a number of mathematical fields in which $A_\infty$-structures arise, ranging from topology to mathematial physics. To study these structures, different interpretations of $A_\infty$-algebras have been given. From the original definition in 1968 \cite{STASHEFF}, to alternative definitions in terms of tensor coalgebras KELLER AND PENKAVA,  approaches that use the machinery of operads \cite{LRW}, \cite{lodayvallette} or certain Lie brackes \citee{RW} to obtain these algebras. 

$A_\infty$-structures have also been combined with brace structures \cite{GV}, MARKL (SOMEWHERE THERE WAS A BRACE OF MARKL) which involved certain signs. Typically, these signs were different for each author and no satisfactory explanaition was given. 

Here we used an operadic approach to obtain these signs in a more conceptual and consistent way. As a consequence, we will generalize the Lie bracket used in R-W and will give a very simple interpretationn of $A_\infty$-algebras. The difference between our operadic approach and others mentioned before is that ours uses much more elementary tools. We hope that this provides a useful way of thinking about derived $A_\infty$-structures. 

The text is organized as follows. REFERENCES TO SECTIONS In Section 1 we recall some basic definitions and establish some conventions. In Section 2 we define a device called \emph{operadic suspension} that will help us obtain the signs that we want and link this device to the classical operadic approach to $A_\infty$-algebras. We also take this construction to the level of the underlying collections of the operads to also obtain a nice description of $\infty$-morphisms of $A_\infty$-algebras. In Section 3 we study the brace algebra induced by operadic supension and show that as a particular case we obtain the Lie bracket from R-2. 
...
Let $A$ be a graded module. Let $S(A)$ be the graded module with $S(A)^v=A^{v+1}$, and so the suspension or \emph{shift} map $S:A\to S(A)$ given by the identity map has internal degree $-1$.
 ...
 Let $f\in \End_A(N)^i=\hom(A^{\otimes N},A)^i$. Recall that REFERENCE TO ITS APPEARENCE (POSSIBLY CHANGE THE NOTATTION OF THE FIRST TIME TO MATHFRAK S SIGMA) $\sigma(f)$ is defined as the map making the following diagram commutative
\[
\begin{tikzcd}
