...
Our base category is the category of graded $R$-modules and graded maps, where $R$ is a commutative ring with unit of characteristic distinct of 2. All tensor products are taken over $R$. We denote the $i$-th degree component of $A$ as $A^i$. If $a\in A^i$ we write $\deg(a)=i$. The $R$-modules $\Hom_R(A^{\otimes n},A)$ are naturally graded by $\Hom_R(A^{\otimes n},A)^i=\prod_k\Hom_R((A^{\otimes n})^k,A^{k+i})$ %NOT SURE IF SUM OR PROD %I think it's prod because it's all homomorphisms that raise the degree by i
SHIFT OR SUSPENSION  OPERATION (MAYBE AS A FUNCTOR) POSSIBLY CONNECT WITH THE DEFINITION OF AINFTY ALGEBRA SAYING THAT EQUIVALENTY AN EQUATION  WITHOUT SIGNS ON SUSPENSION (MAYBE REFERENCE TO KELLER)
\begin{defin}
...
LEFT MODULE AND RIGHT INFINITESIMAL MODULE
...
If $\OO$ is an $\mathbb{S}-$module and the insertion maps satisfy some additional axioms regarding the symmetric group action, we say that $\OO$ is a symmetric operad. 

A map of operads (resp. symmetric operads) is a map of collections (resp. $\mathbb{S}-$modules) that is compatible with insertions.
\end{defin}

\begin{defin}
The \emph{endomorphism operad} $\End_A$ is given by the modules \[\End_A(n)=\hom(A^{\otimes n},A)\] for some graded $R$-module $A$. Insertion maps are given by
...
This is exactly the sign in \cite{RW} from which the sign in the equation defining $A_\infty$-algebras (equation (\ref{ainftyequation}) is derived.
...
MENTION THAT REPLACING END BY ANY OPERAD O IN THE CLASSICAL DEFINITION ONE OBTAINS THE NOTION OF AINFTY MULTIPLICATION
