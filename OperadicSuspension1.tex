...
INCLUDE THE MONOIDALITY RESULT HERE AND SAY SOMETHING ABOUT THE RESULTS THAT ARE MENTIONED
...
Our base category is the category of graded $R$-modules and graded maps, where $R$ is a commutative ring with unit of characteristic distinct of 2. All tensor products are taken over $R$. We denote the $i$-th degree component of $A$ as $A^i$. If $a\in A^i$ we write $\deg(a)=i$. The $R$-modules $\Hom_R(A,B)$ are naturally graded by \[\Hom_R(A,B)^i=\prod_k\Hom_R((^k,B^{k+i}).\]

We adopt the following Koszul sign convention: for $a\in A$, $b\in B$, $f\in\Hom_R(A,C)$ and $g\in\Hom_R(B,D)$

\[(f\otimes g)(a\otimes b)=(-1)^{\deg(a)\deg(g)}f(a)\otimes g(b).\] %NOT SURE IF SUM OR PROD %I think it's prod because it's all homomorphisms that raise the degree by iFFFLVVVV
...
\end{defin}
\begin{defin}
An $A_\infty$-algebra is a graded $R$-module $A$ together with maps $m_n:A^{\otimes n}\to A$ of degree $2-n$ satisfying for all $n\geq 1$ the equation
...
\[(\OO\circ\PP)(n)=\bigoplus_{k\geq 0}\OO(k)\otimes \left(\bigoplus_{i_1+\cdots+i_k=n} \PP(i_1)\otimes\cdots\otimes \PP(i_k)\right).\]
\end{defin}
...
\[(f\circ g)(a\otimes b_1\otimes\cdots\otimes b_k)=(-1)^{\varepsilon} f(a)\otimes g(b_1)\otimes\cdots\otimes g(b_k),\]
...
for each $1\leq i\leq n$ satisfying natural uunitality and associativity axioms. Insertion maps can be iterated to define \emph{composition maps} \[\gamma(a;b_1,\dots, b_n)=(\cdots(a\circ_1 b_1)\circ_2 b_2\cdots
)\circ_n b_n).\]
...
This is exactly the sign in \cite{RW} from which the sign in the equation defining $A_\infty$-algebras (\cref{ainftyequation}) is derived.
...
\[a\tilde{\circ}b=\sum_{i=1}^n(-1)^{(n-1)(m-1)+(n-1)\deg(b)+(i-1)(m-1)}a\circ_i b\]
for $a\in\OO(n)$ and $b\in\OO(m)$. Therefore, applying this definition to $m_{r+1+t}$ and $m_s$ we obtain that
...
Recall from definition \ref{collections} that an $\mathbb{S}$-module is a a collection $\{\OO(n)\}_{n\geq 0}$ equipped with a right action of the symmetric groups. 
...
Note that $\deg(a)+n-1$ is the degree of $a\otimes e^n$ and as we said $\deg(\s f)=\deg(f)$, so the above relation is consistent with the Koszul sign rule. In any case, recall that a morphism of operads is necessarily of degree 0, but the above calcultion hints some monoidality properties of $\s$ that we will study afterwards. Clearly $\s f$ preserves the unit, so $\s f$ is a morphism of operads. 
...
\begin{propo}
The functor $\s$ is an equivalence of categories both at the level of collections (or $\mathbb{S}$-modules) and at the level of operads. \qed %it is not an isomorphism because at the level of collections, not every collection is EQUAL to some suspension
\end{propo}
...
Defining $\mathfrak{s}$ at the level of collections also allows us to talk about $\infty$-morphisms of $A_\infty$-algebras in this setting, since they live in collections of the form\[\End^A_B=\{\Hom(A^{\otimes n},B)\}_{n\geq 1}.\] More precisely, there is a left module structure on $\End^A_B$ over the operad $\End_B$
...
\subsection{Monoidal properties of operadic suspension}
Now we are going to explore the monoidal properties of operadic suspension.

Since operads are precisely monoids on the category $\mathbb{S}$-Mod of graded $\mathbb{S}$-modules, we have the following.
\begin{propo}
...
\begin{propo}
The functor $\s:\mathbb{S}\mbox{-Mod}\to \mathbb{S}\mbox{-Mod}$ defines a lax monoidal functor. When restricted to the subcategory of reduced operads, it is strong monoidal.
\end{propo}
\begin{proof}
Firstly, we need to define the structure maps of a lax monoidal functor. Namely, we de define the unit morphism $\varepsilon:I\to\s I$ to be the map $\varepsilon(n):I(n)\to I(n)\otimes S^{n-1}k$ to be the identity for $n\neq 1$ and the isomorphism $k\cong k\otimes k$ for $n=1$. We also need to define a natural transformation $\mu:\s\OO\circ\s\PP\to\s(\OO\circ\PP)$. To define it, observe that for $\PP=\OO$ we would want the map
...
\[\s\OO\circ\s\OO\xrightarrow{\mu}\s(\OO\circ\OO)\xrightarrow{\s\gamma}\s\OO\]
 ...
 As we show in \Cref{bracesign} $\tilde{\gamma}=(-1)^\eta\gamma$, $\s\gamma$ does not add any signs, so the sign must come entirely from the map $\s\OO\circ\s\OO\to\s(\OO\circ\OO)$. Therefore, we define the map \[\mu:\s\OO\circ\s\PP\to\s(\OO\circ\PP)\] as the map given by
 \[a\otimes e^k\otimes b_1\otimes e^{i_1}\otimes\cdots\otimes b_k\otimes e^{i_k}\mapsto (-1)^\eta a\otimes b_1\otimes\cdots\otimes b_k \otimes e^n,\]
 where $i_1+\cdots+i_k=n$ and 
 \[\eta=\sum_{j<l}i_j\deg(b_l)+\sum_{j=1}^k (i_j+\deg(b_j)-1)(k-j),\]
...
\end{tikzcd}\]
 Let $c=a\otimes e^k\otimes b_1\otimes e^{i_1}\otimes\cdots\otimes b_k\otimes e^{i_k}\in \mathfrak{s}\mathcal{O}\circ\mathfrak{s}\mathcal{P}$ and let us compute $\s(f\circ g)(\mu(c))$. 
 
\[\s(f\circ g)(\mu(c))=\s(f\circ g)((-1)^{\sum_{j<l}i_j\deg(b_l)+\sum_{j=1}^k (i_j+\deg(b_j)-1)(k-j)} a\otimes b_1\otimes\cdots\otimes b_k \otimes e^n)=\]
\[(-1)^{\nu}f(a)\otimes g(b_1)\otimes\cdots\otimes g(b_k) \otimes e^n\]
where
\[\nu=\sum_{j<l}i_j\deg(b_l)+\sum_{j=1}^k (\deg(b_j)+i_j-1)(k-j)+k\deg(g)\deg(a)+\deg(g)\sum_{j=1}^k\deg(b_j)(k-j).\]
...
\[\mu((\s f\circ \s  g)(c))=(-1)^\sigma f(a)\otimes g(b_1)\otimes\cdots\otimes g(b_k) \otimes e^n,\]
where 
\[\sigma=k\deg(g)(\deg(a)+k-1)+\deg(g)\sum_{j=1}^k(\deg(b_j)+i_j-1)(k-j)+\]\[\sum_{j<l}i_j(\deg(b_j)+\deg(g))+\sum_{j=1}^k(i_j+\deg(b_j)+\deg(g)-1)(k-j).\]
 
 Now we compare the two signs by computing $\nu+\sigma\mod 2$. After some cancellations of common terms and using that $k(k-1)=0\mod 2$ we get
 
 \[\deg(g)\sum_{j=1}^k(i_j-1)(k-j)+\sum_{j<l}i_j\deg(g)+\sum_{j=1}^k\deg(g)(k-j)=\]
 \[\deg(g)\sum_{j=1}^ki_j(k-j)+\deg(g)\sum_{j<l}i_j=\]
 \[\deg(g)\left(\sum_{j=1}^k i_j(k-j)+\sum_{j=1}^k i_j(k-j)\right)=0\mod 2.\]
 
 This shows naturality of $\mu$. Next, we have to show that $\mu$ and $\varepsilon$ satisfy the axioms of a lax monoidal functor. Unitality follows easily from the definitions. In the case of associativity, oberve that by the definition of $\mu$, the assoociativity axiom is equivalent to the associativity of the operadic composition $\tilde{\gamma}$ which we know to be true. This shows that $\s$ is a lax monoidal functor.
...
\[\Lambda(n)\to \bigoplus_{k\geq 0} \Lambda(k)\otimes\left(\bigoplus_{i_1+\cdots+i_k=n}\Lambda(i_1)\otimes\cdots\otimes\Lambda(i_k)\right)\]
as the map
\[e^n\mapsto\sum_{i_1+\cdots+i_k=n}(-1)^{\delta}e^k\otimes  e^{i_1}\otimes\cdots\otimes e^{i_k},\]
where $\delta$ is just the same sign that shows up in the operadic composition on $\Lambda$ (see \Cref{bracesign}) and $i_1,\dots,i_k>0$. Since there are only finitely many ways of decomponsing $n$ into $k$ positive integers, the sum is finite and thus the map is well defined. In fact, this map defines a cooperad structure on the reduced sub-operad of $\Lambda$ with trivial arity 0 component. This map induces the morphism $\mu^{-1}:\s(\OO\circ\PP)\to \s\OO\circ\s\PP$ that we are looking for.
...
For $f\in \s\OO(N)$ and $g_i\in\s\OO(a_i)$ of internal degree $q_i$ ($1\leq i\leq n$), we have
...
Finally, as we mentioned before in Section \ref{functorial}, we can show the following alternative description of $\infty$-morphisms of $A_\infty$-algebras and their composition in terms of suspension of collections (recall definition \ref{infinitymorphism}).
...
where \[\lambda:\mathfrak{s}\End_B\circ \mathfrak{s}\End^A_B\to \mathfrak{s}\End^A_B\] is induced by the left module struncture on $\End^A_B$ and \[\rho:\mathfrak{s}\End_B\circ_{(1)}\mathfrak{s}\End^A_B\to \mathfrak{s}\End^A_B\] is induced by the right infinitesimal module structure on $\End^A_B$ (see Section \ref{functorial} to recall these module structures ). 
...
we know that this equation coincides with the one defining $\infty$-morphisms of $A_\infty$-algebras (Definition \ref{inftymorphism}) up to sign. The signs that appear in the above equation are obtained in a similar way to that on $\tilde{\gamma}$ (see the proof of \ref{bracesign}). Thus, it is enough to plug on $\eta$ (the sign from \ref{bracesign}) the corresponding degrees and arities to obtain the desired result. The composition of $\infty$-morphisms follows similarly.
\end{proof}
...
\end{appendices}
%\phantomsection
USE MATHSCINET OR OTHER SOURCES TO FIND APPROPRIATE CITATIONS IF POSSIBLE
