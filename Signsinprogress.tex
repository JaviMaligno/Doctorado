...
We define $x\cdot y=(-1)^{\sigma}m\{x,y\}$ where $m\in C^{2,0}(V,V)$ such that $m\circ m=m\{m\}=m(1,m)-m(m,1)=0$ I MIGHT PUT A MINUS SIGN IN FRONT JUST TO GET $M(M,1)-M(1,M)$  INSTEAD BUT WHAT I GOT IS WHAT CONSTANZE HAS and $\sigma$ is the sign corresponding to the definition of the brace. More precisely, we have
...
\begin{align*}
(x\cdot y)\cdot z=(-1)^{a(x)+q(x)-1+a(y)q(y)+a(x)+a(y)+q(x)+q(y)-1+a(z)q(z)}m\{m\{x,y\},z\}=\\(-1)^{a(y)q(y)+a(y)+q(y)+a(z)q(z)}m\{m\{x,y\},z\}
\end{align*}
and

\begin{align*}x\cdot (y\cdot z)=(-1)^{a(x)+q(x)-1+(a(y)+a(z))(q(y)+q(z))+a(y)+q(y)-1+a(z)q(z)}m\{x,m\{y,z\}\}=\\
(-1)^{a(x)+q(x)+a(y)q(y)+a(y)q(z)+a(z)q(y)+a(y)+q(y)}m\{x,m\{y,z\}\}
\end{align*}
I DIDN'T GET THESE SIGNS ON PAPER, I GOT THE ONES THAT MAKE THE BRACE EQUATION HOLD, SO CHECK THIS (I KNOW I DID IT BY EXTRACTING THE SIMPLIFIED SIGN HERE AND BY SIMPLIFYING IT AFTERWARDS ON PAPER)
...
$$\binom{a(x)+a(y)+a(z)}{2}+a(x)+q(x)-1+\binom{a(x)}{2}+\binom{a(y)}{2}+\binom{a(z)}{2}-a(x)(a(y)+q(y)+a(z)+q(z))-a(y)(a(z)+q(z))\equiv$$
$$a(x)+q(x)-1+a(x)a(y)+a(x)a(z)+a(y)a(z)+a(x)a(y)+a(x)q(y)+a(x)a(z)+a(x)q(z)+a(y)a(z)+a(y)q(z)=$$
$$a(x)+q(x)-1+a(y)a(z)+a(x)q(y)+a(x)q(z)+a(y)a(z)+a(y)q(z)$$
...
$$m\{m\}\{x,y,z\}=(-1)^{a(x)+q(x)-1+a(y)a(z)+a(x)q(y)+a(x)q(z)+a(y)a(z)+a(y)q(z)}(m(x,m(y,z))-m(m(x,y),z)))$$

On the other hand, the koszul sign here is positive because $m$ has degree 0
\end{proof}
\end{document}
