...
We define $x\cdot y=(-1)^{\sigma}m\{x,y\}$ where $m\in C^{2,0}(V,V)$ such that $m\circ m=m\{m\}=m(1,m)-m(m,1)=0$ I MIGHT PUT A MINUS SIGN IN FRONT JUST TO GET $M(M,1)-M(1,M)$ INSTEAD  and $\sigma$ is the sign corresponding to the definition of the brace. More precisely, we have
