...
We define $x\cdot y=(-1)^{a(x)+q(x)-1}m\{x,y\}$ where $m\in C^{2,0}(V,V)$ such that $m\circ m=m\{m\}=m(1,m)-m(m,1)=0$  and %I MIGHT PUT A MINUS SIGN IN FRONT JUST TO GET $M(M,1)-M(1,M)$  INSTEAD BUT WHAT I GOT IS WHAT CONSTANZE HAS and $\sigma$ is the sign corresponding to the definition of the brace. More precisely, we have
