...
&\underset{k-i\geq 1}{\sum_{k,i}}\sum b_{k-i+1}(m;-, b_i(x;-),-)
\end{align*}
...
We now look at the index $k-i+1=l$ what determines the arity of the map $b_{k-i+1}(m;\dots)$. We show that for each value of $l$, $b_1(M_{k-i+1};\Phi(x))$ produces exactly the terms involving $b_{k-i+1}(m;\dots)$. 
