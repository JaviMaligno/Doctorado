...
\item<2-> For $f\in C^n(A)$ and $g_i\in C^{m_i}(A)$ ($i=1,\dots, j$) define the \emph{brace} $b_j(f;g) \in C^{n+\sum m_i-j}(A)$ as
...
\begin{lem}
The operation $[f,g]=f\circ g-(-1)^{\deg(f)}g\circ f$ defines a Lie bracket on $C^*(A)$. 
\end{lem}\pause

\begin{defi}
Let $A$ be a dg algebra. Its \emph{Hochschild complex} is given by $C^n(A)=\hom_R(A^{\otimes n},A)$ with the differential %I like C^0 as R -> A
\[
Df = [d+m,f] = d\circ f - (-1)^{\deg(f)|}f\circ d + m\circ f - (-1)^{\deg(f)}f\circ m %the jacobi identity guarantees that this is a differential
\]
