...
\frametitle{Braces}
\begin{itemize}
\item For $a\in\mathcal{O}(n)$ and $b_1,\dots,b_j\in\mathcal{O}$ ($j\leq n$), define maps called \textbf{braces}
...
\item<3-> The collection $C^*(K)=\{C^*(K_n)\}$ is an operad of chain complexes with insertion maps induced from those of $\{K_n\}$.
...
\end{defi}\pause
\begin{defi}
Let $A$ be a dga. A \emph{termwise $R$-projective
...
\begin{itemize}
\item An $A_\infty$-algebra is \emph{minimal} if $m_{01} = 0$. 
\end{itemize}\pause
...
For a bigraded module $A$, there is a one to one correspondance between $A_\infty$-algebras on $\Tot(A)$ and $dA_\infty$-algebras on $A$.
\end{theorem}\pause
\begin{corollary}
Up to shifts, the following maps define an $dA_\infty$-algebra structure on $\Sigma\Tot(\mathfrak{s}\mathcal{O})$
\begin{align*}
