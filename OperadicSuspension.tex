...
Furthermore, since we have an isomorphism of operads $\End_{\Sigma A}\cong \mathfrak{s}^{-1}\End_A$, we will be able to translate the results of Getzler, who works in $\End_{\Sigma A}$, to $\mathfrak{s}^{-1}\End_A$, which is more similar to our situation. This could help us prove the lemmas that we had in mind. In principle, the desuspension has the same signs as the suspension, but if it's convenient we may use the backwards insertion from Ward to get the isomorphism $\mathfrak{s}\mathfrak{s}^{-1}\OO\cong\OO$ in a more straightforward way (recall that there was an non-identity automorphism of $\OO$ in this isomorphism).

\section{$A_\infty$-structure on $\End_{\Sigma\mathfrak{s}\OO}$}

For an element $x\in\Sigma\s\OO$ let us write $||x||$ for its total degree (the natural degree in this case, arity plus internal degree) and $|x|=||x||-1$ for its \emph{reduced degree} (which is the natural degree in $\s\OO$). We had defined the maps 
...
\begin{remark}
Another possibility that I haven't really considered is sending the $\overline{M}_j$ defined on $\s^{-1}\End_{\s\OO}$ to a map defined on $\s\End_{\s\OO}$. The signs are the same and the map consists of adding some exterior products, so it doesn't really modify the map. Maybe this alternative is easier.
\end{remark}

\subsection{Redefining the maps}
I'm going to use the notation $M_j$ for what I've called $M_j'$ before since we are going to be interested only in these new maps. $M_1$ remains unmodified and $M_2$ has already been defined as $M_2(x,y)=(-1)^{|x|}b_2(m;x,y)$. We want to define $M_j$ for $j\geq 3$ such that for each decomposition $n=r+s+t$ we have
...
THE SIGNS OF THE EXTERIOR PRODUCT THAT GOES AWAY BECAUSE IT'S ALWAYS THE SAME

THE DEGREE FOR THE KOSZUL RULE IS THE TOTAL DEGREE
