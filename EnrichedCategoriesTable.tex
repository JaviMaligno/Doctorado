...
PROPOSITION 4.11 FIRST PART OF THE PROOF IN SOME ESPECIFIC CASE (IDEALLY ONE OF THE PAPER)? $\rightarrow$ this is done for Tot in the paper
...
Proposition 4.55 WHY SYMMETRIC MONOIDALITY OF TOT IMPLIES THAT ABOUT THE DIFFERENTIAL, WHICH IS THE DIFERENTIAL ON THE ENRICHED END AS FILTERED COMPLEX $\rightarrow$ it wasn't relevant (ALSO SEE HOW THE N,Z BIGRADING IS NECESSARY FOR THESE PROPERTIES $\rightarrow$ so that Tot is the same as direct sum and it is an iso), I DON'T FULLY UNDERSTAND LAST PARAGRAPH, WHAT HAPPENS IF WE FORGET ABOUT FILTRATIONS HERE? ARE THERE MORE AINFTY STRUCTURES ON TOT(A)? yes, those that can split as bidegree $(i,2-n-i)$ maps (I was originally trying to show that any of those induced a derived Ainfty algebra, but that must be too much)
...
%$f:A_\infty\to\underline{\mathpzc{End}_A}$ this means that $f(A_\infty)=f(F_0A_\infty)\subseteq F_0(\underline{\mathpzc{End}_A})$
...
Let $C$ and $D$ be monoidal categories and $V$ a symmetric monoidal category. We consider the $V$-enriched categories $\underline{C}$ and $\underline{D}$ with the adjunction
\[\Hom_C(A*X,Y)\cong\Hom_V(A,\underline{C}(X,Y))\]
for $A\in V$, $X,Y\in C$, and similarly for $\underline{D}$.  

Let $F,G:C\to D$ a strong monoidal functors over $V$ with a natural transformation $\mu:F\Rightarrow G$ over $V$ such that $\mu_X:FX\to GX$ is an isomorphism. Let $\underline{F},\underline{G}:\underline{C}\to \underline{D}$ the associated enriched functors and $\underline{\mu}:\underline{F}\Rightarrow\underline{G}$ the associated enriched natural transformation whose component $\mu_X:1\to \underline{D}(FX,GX)$ is defined to be the adjoint of $\mu_X:FX\to GX$.

Then we have the following map $f:\underline{D}(GX,Y)\to\underline{D}(FX,Y)$ (this map will correspond to $\underline{D}(\Tot(A^{\otimes n}),\Tot(A))\to \underline{D}(\Tot(A)^{\otimes n},\Tot(A))$)

\[\underline{D}(GX,Y)\cong \underline{D}(GX,Y)\otimes 1\xrightarrow{Id\otimes \underline{\mu}_X}\underline{D}(GX,Y)\otimes \underline{D}(FX,GX)\xrightarrow{c}\underline{D}(FX,Y)\] 
where $c$ is the composition map in the enriched setting and the tensor products are in $V$. The candidate $g$ for the inverse is defined analogously as 
\[\underline{D}(FX,Y)\cong \underline{D}(FX,Y)\otimes 1\xrightarrow{Id\otimes \underline{\mu}^{-1}_X}\underline{D}(FX,Y)\otimes \underline{D}(GX,FX)\xrightarrow{c}\underline{D}(GX,Y).\]

As in other proofs, I would like to take the composition of these maps to a map in $C$ using the adjunction. However, having the tensor product of maps $Id\otimes \underline{\mu}_X$ I don't know how to apply the adjunction here, as I don't have a map of the form $A\to\underline{D}(X,Y)$ but rather of the form  $A\otimes B\to\underline{D}(X,Y)\otimes \underline{D}(W,Z)$. The maps $f$ and $g$ as a hole certainly have adjoints, but I would like to express them in terms of the maps they are composed of, so that I can work with their definitions. Since these maps are defined in terms of adjoints, I cannot think of other way to show that they are inverse to each other.
