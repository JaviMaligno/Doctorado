...
The operadic suspension defined here is a slight modification of the one found in \cite{ward}.
...
$$\gamma(f;h_1,\dots, h_N)=(\cdots(f\tilde{\circ}_1 h_1)\tilde{\circ}_{1+a(h_1)}h_2\cdots)\tilde{\circ}_{1+\sum a(h_p)}h_N,$$
...
\end{align*}
After insertions, the only sign that do not cancel is $(-1)^{(n-1)(m-1)}$. So we need to find a linear automorphism $f$ of $\OO$ such that, for $a\in\OO(n)$ and $b\in\OO(m)$,
